...
var stackChart1 = dc.barChart("#stack-bar-chart1");
var stackChart2 = dc.barChart("#stack-bar-chart2");

//define variable for x axis value in bar charts
var measureNames=[],doctorsNames = [];

//define the array for tooltip in max denominator numerator
var max_deno_num = [];
...
function generateCharts(){
            ...
            //======================== Stack bar Chart Variables for max denominator and numerator =============
            var lucidStackMaxDenominatorNumeratorDimension = lucidFilterData.dimension(function (d) { return d.Description; });            
            var lucidStackMaxDenominatorNumeratorDimensionGroup = lucidStackMaxDenominatorNumeratorDimension.group().reduce(
                        //add
                        function(p,v){
                                p.Description = v.Description
                                p.Max_denominator.push(parseInt(v.Denominator))
                                p.Max_numerator.push(parseInt(v.Numerator))
                                return p;
                        },
                        //remove
                        function(p,v){
                                p.Description = v.Description
                                p.Max_denominator.splice(p.Max_denominator.indexOf(parseInt(v.Denominator)),1)
                                p.Max_numerator.splice(p.Max_numerator.indexOf(parseInt(v.Numerator)),1)                                
                                return p;
                        },
                        //init
                        function(p,v){
                                return {Description: "", Max_denominator : [], Max_numerator :[]};
                        }
            );
...
/*******************************************************************************************************************************************************/
            ...
            stackChartLength= doctorsNames.length*45.5;    
            
            stackChart1. width(stackChartLength)
                      .height(400)
                      .margins({
                            top: 10, 
                            right: 50, 
                      ...
                      .renderTitle(false)    
                      .colors(d3.scale.category20())    
                      .x(d3.scale.ordinal().domain(doctorsNames))
            ...
            stackChart2. width(stackChartLength)
                      .height(400)
                      .margins({
                            top: 10, 
                            right: 50, 
                      ...
                      .dimension(lucidStackMaxDenominatorNumeratorDimension)
                      .transitionDuration(transitionDurationValue)
                      .group(lucidStackMaxDenominatorNumeratorDimensionGroup,"Denominator")
                      .valueAccessor(function(d,i){ return (typeof(d3.max(d.value.Max_denominator)) != "undefined") ? d3.max(d.value.Max_denominator) : 0})
                      .stack(lucidStackMaxDenominatorNumeratorDimensionGroup, "Numerator", function(d,i){
                                    max_deno_num[d.key] = d3.max(d.value.Max_numerator);
                                    return (typeof(d3.max(d.value.Max_numerator)) != "undefined") ? d3.max(d.value.Max_numerator) : 0            
                        })
                      .gap(20)
                      .colors(d3.scale.category20b())
                      .renderTitle(false)
...
/*
 *Function Name : rotateStackChart2Labels
 *@description: Function is used for rotate the label of stack charts on x-axis
 */
function rotateStackChart2Labels()
{               
            stackChart2.selectAll(".x text")
		.attr("transform", function(d) {
			return "translate(" + this.getBBox().height*-1 + "," + parseInt(d.length*3)+ ")rotate(-90)";
