...
var pieNumeChart = dc.pieChart("#dc-nume-pie-graph");
...
var data_table_data, table_filter_data = [];
...
d3.tsv("data/measure.csv", function (che_data) {        
    ...
    var measureVal = lucidFilterData.dimension(function (d) { return (mesure_values[d.Measure_id]); });    
    var measureGroup = measureVal.group().reduce(
	    ...
	    //init
	    function(p,v){
		    return {count: 0, Denominator: 0, Numerator: 0, charges_avg: 0, deno_avg: 0, nume_avg: 0, max_numeraotr:0};
	    }
    );
    // for pieChart
    var lucidPieDimension = lucidFilterData.dimension(function (d,i) {return mesure_values[d.Measure_id];});    
    var lucidPieDimensionGroup = lucidPieDimension.group().reduceSum(function(d) {return d.Denominator});
    
    // for pieChart
    var pieNumeDimension = lucidFilterData.dimension(function (d,i) {return mesure_values[d.Measure_id];});    
    var pieNumeDimensionGroup = pieNumeDimension.group().reduceSum(function(d) {return d.Numerator});
        
    //define for row chart 
    var lucidRowChartDimension = lucidFilterData.dimension(function(d){return (mesure_values[d.Measure_id]); });       
    var lucidRowChartDimensionGroup = lucidRowChartDimension.group();
    //Make code for get minimum and maximum average 
    
    measures = denominator_data = d3.nest().key(function(d) { return mesure_values[d.Measure_id]; }).entries(lucidData)
    denominator_data = d3.nest().key(function(d) { return mesure_values[d.Measure_id]; }).rollup(function(d) { return d3.sum(d, function(g) {return g.Denominator; }); }).entries(lucidData);
    numerator_data = d3.nest().key(function(d) { return mesure_values[d.Measure_id]; }).rollup(function(d) { return d3.sum(d, function(g) {return g.Numerator; }); }).entries(lucidData);       
    ...
    {
	max_denominator.push(parseFloat(denominator_data[j].values/29))
	max_numerator.push(parseFloat(numerator_data[j].values/29))	
    }   
    ...
    .x(d3.scale.linear().domain([d3.min(max_denominator)-1000, d3.max(max_denominator)+1000]))
    .y(d3.scale.linear().domain([d3.min(max_numerator)-1000, d3.max(max_numerator)+1000]))
    ...
    //Pie Chart with respect to doctor
    pieChart.width(200)
    .height(200)
    .transitionDuration(transitionDurationValue)
    ...
    //Numerator chart
    pieNumeChart.width(200)
    .height(200)
    .transitionDuration(500)
    .dimension(pieNumeDimension)
    .group(pieNumeDimensionGroup)
    .radius(90)    
    .minAngleForLabel(0.5)    
    .label(function(d) {	
        return d.key + ": " + d.value;
    ...
    //Row chart with respect to measure values
    rowChart.width(350)
    .height(800)
	...
	svg.append("text")
		...
		//console.log(p);
		    ...
		    return "<span style='color: #FFF'>" + p.key + "</span><br/>\n\
                            <span style='color: #FFF'>Avg. Denominator: " + parseInt(p.value.deno_avg) + "</span><br/>\n\
                            <span style='color: #FFF'>Avg. Numerator: " + parseInt(p.value.nume_avg) + "</span><br/>"
		//}
	    });
	    ...
	    .html(function (p) {
		  ...
		  $('.d3-tip').css("background",$(this).attr('fill'));
		  $('.d3-tip').after($('.d3-tip').css("color",$(this).attr('fill')));
		  return "<span style='color: #FFF'>" + p.key + "</span><br/><span style='color: #FFF'>Total Patients:" + p.value + "</span>"
	      //}
	    });
	...
	setTimeout(function(){	    	    	
	    table_filter_data_measure= [];
	    if($("#dc-pie-graph g.selected").length > 0 || $("#dc-nume-pie-graph g.selected").length > 0){
		$.each($("#dc-pie-graph g.selected"), function(k,v) {
		    text = $(v).text().split(':');		    
		    if(table_filter_data_measure.indexOf(text[0]) < 0){
			table_filter_data_measure.push(text[0]);		    
		    }		
		});
                $.each($("#dc-nume-pie-graph g.selected"), function(k,v) {
		    text = $(v).text().split(':');		    
		    if(table_filter_data_measure.indexOf(text[0]) < 0){
			table_filter_data_measure.push(text[0]);		    
		    }		
