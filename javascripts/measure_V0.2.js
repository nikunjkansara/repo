...
var stackChartTotalDenominatorNumerator = dc.barChart("#total_denominator_numerator_stack");
var stackChartMaxMinDenominatorNumerator = dc.barChart("#max_min_denominator_numerator_stack");
...
d3.tsv("data/measure.csv", function (che_data) {    
    ...
    DoctorFilterData = che_data.filter(function(d){ return (d.Numerator != 0 && d.Denominator != 0)});        
    ...
    var measureGroup = measureVal.group().reduce(
	    ...
	    //init
	    function(p,v){
		    return {count: 0,Description : "", Denominator: 0, Numerator: 0, charges_avg: 0, deno_avg: 0, nume_avg: 0, max_numeraotr:0, Region_id:0};
	    }
    );         
    ...
    //======================== Stack Chart Variables ====================================
            ...
            var lucidStackDimension = lucidFilterData.dimension(function (d,i) {  return d.Description;});
            ...
            var lucidStackTotalDenominatorGroup = lucidStackDimension.group().reduceSum(function(d){ return d.Denominator})            
            var lucidStackTotalNumeratorGroup = lucidStackDimension.group().reduceSum(function(d){ return d.Numerator })             
            console.log(lucidStackTotalDenominatorGroup);
            console.log(lucidStackTotalNumeratorGroup);
            
            var lucidStackMaxDimension = lucidFilterData.dimension(function (d) { return d.Description; });            
            var lucidStackMaxDenominatorGroup = lucidStackMaxDimension.group().reduce(
                        //add
                        function(p,v){                                                                                    
            ...
            var lucidStackMaxNumeratorGroup = lucidStackMaxDimension.group().reduce(
                        //add
                        function(p,v){                                                                                    
    ...
    var lucidRowChartDimension = lucidFilterData.dimension(function(d){ return mesure_values[d.Measure_id]; });       
    var lucidRowChartDimensionGroup = lucidRowChartDimension.group().reduceSum(function(d){ return d.Denominator});
    ...
    //=========================Stack chart, Max Denominator and Max Numerator , Total Denominator And Total Numerator  with respect to measure id==============================
    //get width for volumeChart based on no. of record
    stackChartLength= measureNames.length*30;    
    
    //stack chart with total  no. of denominator and numerator
    stackChartTotalDenominatorNumerator.width(stackChartLength)
    .height(300)
    .margins({
        top: 10, 
        right: 50, 
    ...
    .transitionDuration(transitionDurationValue)
    .group(lucidStackTotalDenominatorGroup,"Denominator")
    .stack(lucidStackTotalNumeratorGroup,"Numerator")    
    .gap(10)    
    .renderTitle(false)    
    .colors(d3.scale.category20())    
    ...
    //stack chart with maximum and minimum no. of denominator and numerator
    stackChartMaxMinDenominatorNumerator. width(stackChartLength)
    .height(300)
    .margins({
        top: 10, 
        right: 50, 
    ...
    .dimension(lucidStackMaxDimension)
    .transitionDuration(transitionDurationValue)
    .group(lucidStackMaxDenominatorGroup,"Denominator")
    .stack(lucidStackMaxNumeratorGroup,"Numerator")    
    .gap(10)    
    .renderTitle(false)    
    .colors(d3.scale.category20())    
    .x(d3.scale.ordinal().domain(measureNames))
   ...
   createDataTable();
    ...
    //rotate the x axis text of stackChartTotalDenominatorNumerator
    ...
    //this if for append text title on y and x axis in bubble chart after some time of render all charts    
     ...
     //function is called for create all filteration code for data table
...
function dataTableFilteration()
{
	...
	setTimeout(function(){	    	    	
		...
		$.each($(".row  rect.selected"), function(k,v) {                        
		    if(table_filter_data_measure.indexOf($(v).parent().text()) < 0){
			table_filter_data_measure.push($(v).parent().text());		    
		    }		
		});	    
	    }	    
	    createDataTable();
    ...
    $(".stack_chart ._0 rect, .stack_chart ._1 rect").on('click' , function(){
	setTimeout(function(){	    	    	
	    table_filter_data_doctor= [];
	    if($(".stack_chart ._0 rect.selected").length > 0){		
		$.each($(".stack_chart ._0 rect.selected"), function(k,v) {
                        
                    search_text = ($(v).children('label').first().text()).split(':');
		    if(table_filter_data_doctor.indexOf(search_text[0]) < 0){
			table_filter_data_doctor.push(search_text[0]);
		    }		
		});	    
	    }            
	    createDataTable();
...
function createSVGTextAndTollTip()
{
	...
	svg.append("text")
	    ...
	    d3.selectAll(".stack_chart ._0 rect, .stack_chart ._1 rect").call(toolTipStackBar);	    
	    d3.selectAll(".stack_chart ._0 rect,.stack_chart ._1 rect").on('mouseover', toolTipStackBar.show).on('mouseout', toolTipStackBar.hide);    
...
{
            //rotate the x axis text of stackChartTotalDenominatorNumerator
            stackChartTotalDenominatorNumerator.selectAll(".x text")
            .attr("transform", function(d) {
                    return "translate(" + this.getBBox().height*-1 + "," + parseInt(d.length*3)+ ")rotate(-90)";
                });
            //rotate the x axis text of stackChartMaxMinDenominatorNumerator
            stackChartMaxMinDenominatorNumerator.selectAll(".x text")
            .attr("transform", function(d) {
                    return "translate(" + this.getBBox().height*-1 + "," + parseInt(d.length*3)+ ")rotate(-90)";
                });
...
{    
    ...
    rowChart.filterAll();
    stackChartMaxMinDenominatorNumerator.filterAll();
    stackChartTotalDenominatorNumerator.filterAll();   
    dc.renderAll();
    dc.redrawAll();
